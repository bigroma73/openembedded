...
from enigma import eTimer, ePoint, iServiceInformation
...
class SecondInfoBar(Screen):
	...
	def __init__(self, session):
		...
		if BC:
			self["videobit"] = StaticText("Video: N/A kbit/s")
			self.videoBitrate = None
			self["audiobit"] = StaticText("Audio: N/A kbit/s")
			self.audioBitrate = None
			self.onShow.append(self.BitCalc)
		else:
			self["videobit"] = StaticText("Install")
			self["audiobit"] = StaticText("BitrateViewer")
		if GP2test:
	...
	def getVideoBitrateData(self,value, status): # value = rate in kbit/s, status ( 1  = ok || 0 = nok (zapped?))
		if status:
			self["videobit"].text = "Video: %d kbit/s" % value;
		else:
			self.videoBitrate = None

	def getAudioBitrateData(self,value, status): 
		if status:
			self["audiobit"].text = "Audio: %d kbit/s" % value;
		else:
			self.audioBitrate = None
